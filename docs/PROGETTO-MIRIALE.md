...
- **Recensioni**: **10 recensioni verbatim reali complete** (pro/contro, punteggio, data, tipo viaggiatore) in **`lib/reviews.tsx`** (`REVIEWS` + `REVIEW_STATS`). Conteggio **122 → 129** ovunque. NB: Booking espone a testo integrale solo 10 recensioni (JSON `FeaturedReview` nella pagina hotel); le altre 119 hanno solo aggregato/sub-punteggi pubblici. Mostrate come **carosello** (`ReviewsCarousel` + `ReviewCard`) in home e pagine appartamento, + **pagina dedicata `/recensioni`** (`AllReviews.tsx`) stile Booking con sub-punteggi a barre e filtri (tipo viaggiatore, argomenti, ricerca, ordina). Link in footer/Contatti.
